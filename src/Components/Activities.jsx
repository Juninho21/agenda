...
const Activities = () => {
    ...
    const fetchCompanySettings = async () => {
        ...
        try {
            ...
            if (user) {
                const q = query(collection(db, "company_settings"), where("user_id", "==", user.uid));
    ...
    const startExecutionFromState = async (state) => {
        ...
        if (state.clientName) {
            try {
                const user = auth.currentUser;
                let qArgs = [collection(db, "clients"), where("name", "==", state.clientName)];
                if (user) {
                    qArgs.push(where("user_id", "==", user.uid));
                }
                const q = query(...qArgs);
                const querySnapshot = await getDocs(q);
            ...
            <div className="calendar-app" style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg-color)' }}>
                ...
                {currentScreen !== 'home' && (
                    <div style={{
                        ...
                        ></i>
                        <h3 style={{ margin: 0, fontSize: '1.6rem', color: 'var(--text-primary)', fontWeight: '700' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '15px' }}>
                    ...
                    {currentScreen === 'products' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                            <div style={{ background: 'var(--bg-element)', padding: '15px', borderRadius: '10px' }}>
                                <h4 style={{ margin: '0 0 15px 0', color: 'var(--text-primary)', fontSize: '1.6rem' }}>Registrar Produto</h4>
                                <div style={{ display: 'flex', gap: '10px', flexDirection: 'column' }}>
                                    <select
                                        value={currentProduct.name}
                                        onChange={(e) => setCurrentProduct(prev => ({ ...prev, name: e.target.value }))}
                                        style={{ padding: '15px', borderRadius: '12px', border: '1px solid #ccc', fontSize: '1.4rem', background: 'var(--bg-color)', color: 'var(--text-primary)', width: '100%' }}
                                    >
                                    ...
                                    <div style={{ display: 'flex', gap: '10px' }}>
                                        <input
                                            ...
                                            style={{ flex: 1, padding: '15px', borderRadius: '12px', border: '1px solid #ccc', fontSize: '1.4rem', background: 'var(--bg-color)', color: 'var(--text-primary)' }}
                                        />
                                        <select
                                            value={currentProduct.unit}
                                            onChange={(e) => setCurrentProduct(prev => ({ ...prev, unit: e.target.value }))}
                                            style={{ padding: '15px', borderRadius: '12px', border: '1px solid #ccc', fontSize: '1.4rem', width: '100px', background: 'var(--bg-color)', color: 'var(--text-primary)' }}
                                        >
                                    ...
                                    <button onClick={handleAddProduct} style={{ padding: '15px', background: 'var(--accent-color)', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer', fontSize: '1.4rem' }}>
                                        + Adicionar
                                    </button>
                                </div>
                            </div>
                                ...
                                {osData.products.map((prod, idx) => (
                                    <div key={idx} style={{
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1rem' }}>
                                            <h4 style={{ color: 'var(--text-primary)', fontSize: '1.5rem', margin: 0 }}>{prod.name}</h4>
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', fontSize: '1.1rem', color: 'var(--text-muted)' }}>
                                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                                <span style={{ fontSize: '1rem', color: 'var(--text-primary)', fontWeight: 'bold' }}>Princípio Ativo:</span>
                                                {prod.active_ingredient || '-'}
                                            </div>
                                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                                <span style={{ fontSize: '1rem', color: 'var(--text-primary)', fontWeight: 'bold' }}>Lote:</span>
                                                {prod.batch || '-'}
                                            </div>
                                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                                <span style={{ fontSize: '1rem', color: 'var(--text-primary)', fontWeight: 'bold' }}>Validade:</span>
                                                <span style={{ fontWeight: 'bold', color: '#2196F3' }}>{prod.expiration_date || '-'}</span>
                                            </div>
                                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                                <span style={{ fontSize: '1rem', color: 'var(--text-primary)', fontWeight: 'bold' }}>Registro:</span>
                            ...
                            {osData.products.length > 0 && (
                                ...
                                >
                                    <i className='bx bx-check-circle' style={{ fontSize: '1.4rem' }}></i>
                                    Concluir e Voltar
                                </button>
                            )}
                        </div>
                ...
                {currentScreen === 'home' && (
                    <div style={{ padding: '15px', background: 'var(--bg-element)', borderTop: '1px solid #ddd', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        ...
                        >
                            <i className='bx bx-save' style={{ fontSize: '1.8rem' }}></i> Salvar Serviço
                        </button>
                        ...
                        <div style={{ display: 'flex', gap: '15px' }}>
                            ...
                            >
                                <i className='bx bx-pencil' style={{ fontSize: '1.6rem' }}></i> {signatureData ? 'Assinado' : 'Aprovar'}
                            </button>
                            ...
                            >
                                <i className='bx bx-check-circle' style={{ fontSize: '1.6rem' }}></i> Finalizar
                            </button>
                        </div>
                    </div>
                )}
                    ...
                    showSignatureModal && (
                        <div className="modal-overlay" style={{
                            ...
                            <div style={{
                                ...
                                <h3 style={{ margin: 0, color: '#333', textAlign: 'center', fontSize: '1.6rem' }}>Assinatura do Cliente</h3>

                                <div style={{ background: '#f9f9f9', border: '1px solid #ddd', borderRadius: '8px', overflow: 'hidden' }}>
                                    <SignaturePad onChange={setTempSignature} />
                                </div>

                                <div style={{ display: 'flex', gap: '10px' }}>
                                    <button onClick={saveSignature} style={{ flex: 1, padding: '15px', background: '#2ecc71', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontSize: '1.4rem', fontWeight: 'bold' }}>Confirmar</button>
                                </div>
                                <button onClick={() => setShowSignatureModal(false)} style={{ marginTop: '5px', background: 'transparent', border: 'none', color: '#666', cursor: 'pointer', textDecoration: 'underline', padding: '10px', fontSize: '1.2rem' }}>Cancelar</button>
                            </div>
                        </div>
                    )
                }
